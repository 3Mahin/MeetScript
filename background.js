...
${transcription}`;
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a professional meeting minutes generator. Create clear, structured meeting minutes from transcriptions. Do not include the title("MEETING MINUTES") Use the date of the meeting when the request was sent.'
            },
            {
              role: 'user',
              content: prompt
      ...
      // Create DOCX file content (now returns a Promise<Blob>)
      const docxBlob = await this.createDocxContent(meetingMinutes);
      const currentDate = new Date(Date.now()).toDateString();
  ...
  createDocxContent(meetingMinutes) {
    // Use the docx library to create a real Word document
      ...
      sections: [
        {
          ...
          children: [
            ...
            new Paragraph({
              text: `Generated by Mahin's Recorder Extension`,
              spacing: { after: 50 }
            }),
            new Paragraph({
    ...
    // Return a promise for the .docx blob
